...
router.get("/login", function (req, res, next) {
  ...
  res.render("debug-login", { login_url: url.toString() });
...
router.get("/auth/callback",
  ...
  async function (req, res, next) {
    ...
    if (error) {
      ...
      res.render("debug-callback", { response: json });
    ...
    if (json.id_token) {
      idToken = atob(json.id_token.split(".")[1]);
      idToken = JSON.parse(idToken);
      //output += "<pre>";
      //output += JSON.stringify(JSON.parse(idToken), null, 2);
      //output += "</pre>";
    }
    ...
    res.render("debug-callback", { response: json, id_token: idToken });
  },
